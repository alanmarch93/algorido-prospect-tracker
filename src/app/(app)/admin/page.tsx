...
export default async function AdminPage() {
  ...
  if (profileError) {
    return <ErrorBox message={`Profile lookup failed: ${profileError}`} />;
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen p-8">
        <div className="rounded-xl p-8 max-w-md w-full text-center" style={{ background: "#161927", border: "1px solid #ffa72640" }}>
          ...
          <div className="text-lg font-semibold mb-2" style={{ color: "#f2f4ff" }}>One More Step</div>
          <div className="text-sm mb-4" style={{ color: "#8d9ec7" }}>
            Add <code className="px-1.5 py-0.5 rounded text-xs" style={{ background: "#1e2338", color: "#ffa726" }}>SUPABASE_SERVICE_ROLE_KEY</code> to Vercel environment variables.
          </div>
          <div className="text-xs text-left rounded-lg p-4 space-y-2" style={{ background: "#1e2338", color: "#8d9ec7" }}>
            <div>1. Supabase → Settings → API → copy <span style={{ color: "#ffa726" }}>service_role</span> key</div>
            <div>2. Vercel → project → Settings → Environment Variables</div>
            <div>3. Add <span style={{ color: "#ffa726" }}>SUPABASE_SERVICE_ROLE_KEY</span></div>
            <div>4. Redeploy</div>
          </div>
        </div>
      </div>
  ...
  try {
    ...
    allProspects = (pr.data ?? []) as Prospect[];
  } catch (e) {
    fetchError = e instanceof Error ? e.message : String(e);
  }

  if (fetchError) {
    return <ErrorBox message={`Data fetch failed: ${fetchError}`} />;
  }
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { label: "Total Users",     value: String(byUser.length),  sub: "active trackers",  color: "#3399ff" },
          ...
          return (
            <div key={p.id} className="rounded-xl overflow-hidden" style={{ background: "#161927", border: "1px solid #2d3757" }}>
              <div className="px-5 py-4 flex items-center gap-3 flex-wrap" style={{ borderBottom: "1px solid #2d3757", background: "#1e2338" }}>
                <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0" style={{ background: "#3399ff20", color: "#3399ff" }}>
              ...
              <div className="divide-y" style={{ borderColor: "#2d3757" }}>
                {prospects.slice(0, 5).map(pr => (
                  <div key={pr.id} className="flex items-center gap-3 px-5 py-3 flex-wrap">
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0" style={{ background: "#1e2338", color: "#3399ff" }}>
                      {pr.full_name[0]}
                    </div>
                    <div className="flex-1 min-w-0">
                ...
                {prospects.length > 5 && (
                  <div className="px-5 py-2 text-xs" style={{ color: "#596494" }}>+{prospects.length - 5} more prospects</div>
                )}
              </div>
            </div>
          );
  ...
  return (
    <div className="p-8">
      <div className="rounded-xl p-6" style={{ background: "#ff595915", border: "1px solid #ff595940" }}>
        <div className="font-semibold mb-2" style={{ color: "#ff5959" }}>Error — please send this to support</div>
        <pre className="text-xs whitespace-pre-wrap break-all" style={{ color: "#8d9ec7" }}>{message}</pre>
      </div>
    </div>
  );
}
